// Toggle info box functionality
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const toggleIcon = toggleInfoBox.querySelector('i');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const themesData = [
                {% for theme in themes %}
                {
                    id: '{{ theme.id|escapejs }}',
                    name: '{{ theme.name|escapejs }}',
                    subthemes: [
                        {% for subtheme in theme.subthemes %}
                        {
                            id: '{{ subtheme.id|escapejs }}',
                            name: '{{ subtheme.name|escapejs }}',
                            description: '{{ subtheme.description|default:''|escapejs }}',
                            icon: '{{ subtheme.icon|default:''|escapejs }}',
                            indicators: [
                                {% for indicator_id in subtheme.indicators %}
                                    {% with indicator=indicators.all|get_item:indicator_id %}
                                        {
                                            id: '{{ indicator_id|escapejs }}',
                                            title: '{{ indicator.title|escapejs }}',
                                            description: '{{ indicator.description|escapejs }}',
                                            icon: '{{ indicator.icon|escapejs }}',
                                            map1_url: '{{ indicator.map1_url|default:''|escapejs }}',
                                            map2_url: '{{ indicator.map2_url|default:''|escapejs }}',
                                            posts: '{{ indicator.posts|default:''|escapejs }}',
                                            source: '{{ indicator.source|default:''|escapejs }}',
                                            unit_of_measure: '{{ indicator.unit_of_measure|default:''|escapejs }}',
                                            theme: '{{ theme.id|escapejs }}'  // Add theme ID to each indicator
                    ...
                    description: '{{ theme.description|escapejs }}',
                    icon: '{{ theme.icon|escapejs }}'
{{ ... }}
                }{% if not forloop.last %},{% endif %}
                {% endfor %}
            ];
                ...
                if (subthemeInfo) {
                    subthemeInfo.innerHTML = `
                        <div class="info-box-header">
                            <div class="info-box-title">Sub-theme Information</div>
                            <div class="subtheme-icon">
                                <i class="icon ${subtheme.icon || 'fa-folder-open'}"></i>
                            </div>
                        </div>
                        <div class="text-content">
        ...
        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            ${posts.map(post => {
                                ...
                                return `
                                    <article class="post-card">
                                        ${featuredImage ? `<div class="post-image" data-bg-image="${featuredImage}"></div>` : ''}
            ...
            const currentIndicator = {
                ...
                title: '{{ indicators.page_name.title|escapejs }}'
            };
